...
export default function SuccessView({ onActivate, onBack }: Props) {
  ...
  return (
    <div style={{ height: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
      <div style={{ width: "100%", maxWidth: 400, textAlign: "center" }}>

        <div style={{ width: 72, height: 72, borderRadius: 20, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, margin: "0 auto 20px", boxShadow: "0 0 32px var(--accent-glow)" }}>🎉</div>

        <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 26, letterSpacing: "-0.02em", marginBottom: 10 }}>Thank you so much!</h1>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px", marginBottom: 20, textAlign: "left" }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>Enter your activation code</div>
          <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 14, lineHeight: 1.5, fontFamily: "var(--font-mono)" }}>Got the email already? Paste your code below to unlock Pro now.</p>
        ...
        <button onClick={onBack} className="btn btn-ghost" style={{ width: "100%", fontSize: 13 }}>Back to my scripts</button>
        <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 20, lineHeight: 1.6, fontFamily: "var(--font-mono)" }}>Questions? noreply@leomagazzu.it</p>
      </div>
    </div>
  );
}
