...
export default function SuccessView({ onActivate, onBack }: Props) {
  ...
  const handleActivate = async () => {
    ...
    if (result === "ok") {
      setActivated(true);
      setShowWelcome(true);
      onActivate(trimmed); // saves to localStorage, sets isPro
    } else {
    ...
    <>
      <div style={{ height: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
        <div style={{ width: "100%", maxWidth: 400, textAlign: "center" }}>
          ...
          <div style={{ width: 72, height: 72, borderRadius: 20, background: "var(--surface)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, margin: "0 auto 20px", boxShadow: "0 0 32px var(--accent-glow)" }}>
            {activated ? "✦" : "🎉"}
          </div>

          <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 26, letterSpacing: "-0.02em", marginBottom: 10 }}>
            {activated ? "You're Pro!" : "Thank you so much!"}
          </h1>
          <p style={{ fontSize: 14, color: "var(--text-2)", lineHeight: 1.6, marginBottom: 28 }}>
            {activated
              ? "Your activation was successful. Check your profile to set up sync."
          ...
          <button onClick={onBack} className="btn btn-ghost" style={{ width: "100%", fontSize: 13 }}>
            {activated ? "Go to my scripts" : "Back to my scripts"}
          </button>
      ...
      {showWelcome && <WelcomeModal onClose={() => { setShowWelcome(false); onBack(); }} />}
    </>
  );
}
