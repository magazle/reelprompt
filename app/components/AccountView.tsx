      ...
      <div style={scroller}>
        <div style={{ padding: "0 24px 48px", paddingTop: topPad, maxWidth: 480, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32 }}>
            <button onClick={onBack} style={{ width: 38, height: 38, borderRadius: 12, background: "var(--surface)", border: "1px solid var(--border-2)", color: "var(--text-2)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }} title="Back">
          ...
          {isOffline && (
            <div style={{ padding: "12px 14px", borderRadius: 12, background: "var(--bg-2)", border: "1px solid var(--border-2)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <p style={{ fontSize: 12, color: "var(--text-2)", lineHeight: 1.5, fontFamily: "var(--font-mono)", margin: 0 }}>You're offline. Sync is paused — your scripts are still available locally.</p>
            </div>
          )}
          ...
          <div style={{ background: "var(--surface)", border: "2px solid var(--accent)", borderRadius: 16, padding: "20px", boxShadow: "0 0 32px var(--accent-glow)", marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: user?.email ? 12 : 0 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)" }}>{user ? "Lifetime access · synced across devices" : "Lifetime access · sign in to sync"}</div>
              </div>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px", marginBottom: 14 }}>
              ...
              {syncState === "sent" ? (
                <div style={{ textAlign: "center", padding: "8px 0" }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-3)", lineHeight: 1.5, fontFamily: "var(--font-mono)" }}>We sent a magic link to <strong>{email}</strong>. Tap it to sign in.</p>
                </div>
              ) : (
                <>
                  <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 14, lineHeight: 1.5, fontFamily: "var(--font-mono)" }}>Enter the email you used to activate Pro — we'll send a magic link.</p>
                  <div style={{ display: "flex", gap: 8 }}>
                    <input type="email" placeholder="your@email.com" value={email} onChange={(e) => { setEmail(e.target.value); setSyncError(""); }} onKeyDown={(e) => e.key === "Enter" && handleSendMagicLink()}
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px", marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 16, lineHeight: 1.5, fontFamily: "var(--font-mono)" }}>Love ReelPrompt? A coffee helps keep it going — no minimum, no pressure.</p>
            <a href={KO_FI_DONATE_URL} target="_blank" rel="noopener noreferrer"
              ...
              onMouseLeave={(e) => (e.currentTarget.style.background = "var(--bg-2)")}>☕ Donate freely</a>
          </div>

          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "20px", marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>📬 Contact & Feedback</div>
            <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 14, lineHeight: 1.5, fontFamily: "var(--font-mono)" }}>Change email, report a bug, or just say hi — we read everything.</p>
            {formState === "sent" ? (
              <div style={{ padding: "14px", borderRadius: 10, background: "var(--bg-2)", border: "1px solid var(--border)", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--font-mono)", marginTop: 4 }}>{user?.email ? `We'll get back to you at ${user.email}` : "We'll get back to you soon."}</div>
              ...
              <>
                <textarea placeholder="Your message…" value={message} onChange={(e) => { setMessage(e.target.value); if (formState === "error") setFormState("idle"); }} rows={4}
          ...
          <button onClick={handleSignOut} className="btn btn-ghost" style={{ width: "100%", fontSize: 14, color: "var(--text-3)" }}>Sign out</button>
        </div>
      </div>
    </div>
  );
