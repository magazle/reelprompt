...
// ~50-word passage for WPM calibration
const CAL_TEXT =
...
const CAL_WORDS = CAL_TEXT.trim().split(/\s+/).length;
...
function WPMCalibrator({ currentWpm, onCalibrated }: {
  ...
  const handleDone = useCallback(() => {
    ...
    const wpm = Math.round((CAL_WORDS / seconds) * 60);
    // Map wpm 80–240 → speed 1–10
  ...
  if (state === "idle" || state === "done") return (
    <div style={boxStyle}>
      {currentWpm && (
        <div style={{ fontSize: 12, color: "var(--green)", fontFamily: "var(--font-mono)", marginBottom: 10, display: "flex", gap: 6, alignItems: "center" }}>
          <span>●</span> Calibrated: {currentWpm} WPM — speed auto-set
        </div>
      )}
      {!currentWpm && (
        <p style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.5, marginBottom: 14 }}>
          Read a short passage aloud. The app measures your pace and sets scroll speed automatically.
        </p>
      )}
      <button className="btn btn-ghost" style={{ width: "100%", fontSize: 13 }} onClick={() => setState("ready")}>
        <IconTarget /> {currentWpm ? "Re-calibrate" : "Calibrate my speed"}
      </button>
    </div>
  );
  ...
  if (state === "ready") return (
    <div style={boxStyle}>
      <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 12 }}>
        Read this aloud at your natural pace ↓
      </div>
      <p style={{
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button className="btn btn-ghost" style={{ flex: 1, fontSize: 13 }} onClick={reset}>Cancel</button>
        <button className="btn btn-primary" style={{ flex: 2, fontSize: 13 }} onClick={handleStart}>● Start Reading</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={boxStyle}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--accent)", letterSpacing: "0.08em", textTransform: "uppercase", display: "flex", gap: 6, alignItems: "center" }}>
          <span className="rec-pulse" style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: "var(--accent)" }} />
          Reading…
        </div>
        <span style={{ fontSize: 14, fontFamily: "var(--font-mono)", color: "var(--text-2)" }}>{elapsed}s</span>
      </div>
      <p style={{
        fontSize: 15, lineHeight: 1.8, color: "var(--text)",
        fontFamily: "var(--font-serif)", fontStyle: "italic",
      ...
      <button className="btn btn-primary" style={{ width: "100%", fontSize: 13 }} onClick={handleDone}>
        ✓ Done reading
      </button>
    </div>
  );
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}
      ...
      >
        ...
        <div style={{ padding: "0 24px 0" }}>
          ...
          {/* ── WPM CALIBRATION ── */}
          <SectionLabel>Speed Calibration</SectionLabel>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 8, lineHeight: 1.4 }}>
              Band = dark strip behind text only. Full = darkened overlay. None = text shadow only.
            </p>
          </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {(["top", "center", "bottom"] as const).map((pos) => (
                <button key={pos} onClick={() => set({ position: pos })} style={{
                  ...
                  fontSize: 13, fontWeight: 600, fontFamily: "var(--font-display)", textTransform: "capitalize",
                }}>
                  {pos}
                </button>
              ))}
          ...
          <Toggle label="Mirror Video Preview" sub="Default on for front camera"
